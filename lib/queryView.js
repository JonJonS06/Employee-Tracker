...
function viewDepartments() {
    db.query(`
    SELECT department.id AS deparment_id, department.name AS department_name FROM department`, (err, res) => {
        if (err) throw err;
        console.table(res);
...
function viewEmployees() {
    ...
    department.name AS department, role.salary AS salary, CONCAT(manager.first_name, SPACE(1), 
    manager.last_name) AS manager_name FROM employee
